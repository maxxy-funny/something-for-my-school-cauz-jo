...
local function obfuscateScript(string)
    ...
    local setmathrandom = randomString(10)
    local settrue = randomString(10)
    local setfalse = randomString(10)
    local gaming = randomString(10)
    ...
    local newSetMath = "local "..setmathrandom.." = math\n"
     local newtrue = "local "..settrue.." = true\n"
     local newfalse = "local "..setfalse.." = false\n"
    local newgame = "local "..gaming.." = game\n"
    ...
    string = string.gsub(string, "math", setmathrandom)
    string = string.gsub(string, "true", settrue)
    string = string.gsub(string, "false", setfalse)
    string = string.gsub(string, "game", gaming)
  
    
    --//* Pattern Search End
    --//* Print Obfuscated Script
    print(newSetMt..newgame..newtrue..newfalse..newGetMt..newIPairs..newLoad..newLoadF..newLoadS..newModule..newNext..newPairs..newPcall..newPrint..newSetMath..newGRandom..newVersionRandom..newAssert..newCollectGarbage..newDoFile..newError..newGetFenv..string)
